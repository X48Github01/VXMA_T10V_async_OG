...
def candle(df, symbol, tf):
    ...
    color = mplf.make_marketcolors(
        up="white", down="blue", wick="blue", edge="blue"
    )
    ...
    try:
        vxma = mplf.make_addplot(
            data.vxma, secondary_y=False, color="red", width=1.5
        )  # ,savefig='candle.png'
        bPrice = mplf.make_addplot(
            data.buyPrice,
            secondary_y=False,
            color="green",
            type="scatter",
...
# set leverage
async def setleverage(symbol, exchange, lev):
    try:
...
async def OpenLong(df, balance, symbol, lev, exchange, currentMODE, Lside):
    ...
    leve = await setleverage(symbol, exchange, lev)
    ...
    if free > min_balance:
            ...
            if currentMODE["dualSidePosition"]:
                ...
                if Tailing_SL:
                    ordertailingSL = await exchange.create_order(
                        ...
                        amount,
                        params={
                            "activationPrice": float(RR1(df, True, ask)),
        ...
        if USETP:
            orderTP = await exchange.create_order(
                ...
                "sell",
                amttp1,
                float(RRTP(df, True, 1, ask)),
                params={
                    "stopPrice": float(RRTP(df, True, 1, ask)),
                    "triggerPrice": float(RRTP(df, True, 1, ask)),
                    "positionSide": Lside,
                },
            )
            orderTP2 = await exchange.create_order(
                ...
                "sell",
                amttp2,
                float(RRTP(df, True, 2, ask)),
                params={
                    "stopPrice": float(RRTP(df, True, 2, ask)),
                    "triggerPrice": float(RRTP(df, True, 2, ask)),
                    "positionSide": Lside,
                },
            )
    ...
    if free > min_balance:
        ...
        if USETP:
            orderTP = await exchange.create_order(
                ...
                "buy",
                amttp1,
                float(RRTP(df, False, 1, bid)),
                params={
                    "stopPrice": float(RRTP(df, False, 1, bid)),
                    "triggerPrice": float(RRTP(df, False, 1, bid)),
                    "positionSide": Sside,
                },
            )
            logging.info(orderTP)
            orderTP2 = await exchange.create_order(
                ...
                "buy",
                amttp2,
                float(RRTP(df, False, 2, bid)),
                params={
                    "stopPrice": float(RRTP(df, False, 2, bid)),
                    "triggerPrice": float(RRTP(df, False, 2, bid)),
                    "positionSide": Sside,
                },
            )
            logging.info(orderTP2)
...
async def feed(df, symbol, exchange, currentMODE):
    ...
    if df["BUY"][last]:
        ...
        if not is_in_Long and USELONG:
            ...
            await OpenLong(
                df, balance, symbol, leverage, exchange, currentMODE, Lside
            )
            is_in_Long = True
        else:
    ...
    if df["SELL"][last]:
        ...
        if not is_in_Short and USESHORT:
            await exchange.cancel_all_orders(symbol)
            await OpenShort(
                df, balance, symbol, leverage, exchange, currentMODE, Sside
            )
            is_in_Short = True
        else:
            print("already in position, nothing to do")
